...
import type { CountGroup, Goal, GoalMember, RequirementGroup, Task } from "@/lib/model";
import {
  isBundleSatisfied,
  isGoalSatisfied,
  isGroupSatisfied,
            ...
            <div key={bundle.id}>
              ...
              >
                <div className="mb-2 text-xs font-semibold text-foreground">
                  {bundle.label ?? `Option ${String.fromCharCode(65 + i)}`}
                  {bundleSatisfied && (
  ...
  return (
    ...
    >
      {group.label && (
        <div className="mb-2 text-xs font-semibold text-foreground">
          {group.label}
          {satisfied && <span className="ml-1 text-osrs-green"> ✓</span>}
        </div>
      )}
      {group.goals.map((member) => {
        if ("kind" in member) {
          ...
          return (
            <SubGroupBlock
              key={member.id}
              group={member}
              redundant={redundant || (satisfied && !isGroupSatisfied(member))}
              onIncrementGoal={onIncrementGoal}
              onDecrementGoal={onDecrementGoal}
            />
          );
        ...
        return (
          <GoalRow
            key={member.id}
            goal={member}
            redundant={redundant || (satisfied && !isGoalSatisfied(member))}
            onIncrementGoal={onIncrementGoal}
            onDecrementGoal={onDecrementGoal}
          />
        );
...
function GoalRow({
  ...
  const pct =
    goal.target > 0 ? Math.min(100, (goal.progress / goal.target) * 100) : 100;
  const satisfied = isGoalSatisfied(goal);
  return (
    <div
      className={cn("mb-2.5 last:mb-0", redundant && "opacity-45 grayscale")}
    >
      ...
      <div className="mb-1.5 flex items-center gap-1.5">
        ...
        <Button
          ...
          title={redundant ? "Group already satisfied — no points" : ""}
        >
          +1
        </Button>
        {satisfied && (
