...
export function saveModel(model: Model): void {
    ...
    try {
        window.localStorage.setItem(
            STORAGE_KEY,
            JSON.stringify(deriveSaved(model)),
        );
    } catch {
    ...
    const tasks = seed.tasks.map((task) => ({
        ...
        groups: task.groups.map((group) => {
            if (group.kind === "count") {
                return {
                    ...group,
                    goals: group.goals.map((m) =>
                        applyMember(m, saved.progress),
                    ),
                };
            }
            if (group.kind === "pool") {
                return {
                    ...
                    goals: group.goals.map((goal) =>
                        applyProgress(goal, saved.progress),
                    ),
                };
            }
            return {
...
function applyMember(
    ...
    if ("kind" in member) {
        if (member.kind === "pool") {
            return {
                ...member,
                goals: member.goals.map((g) => applyProgress(g, progress)),
            };
        }
        return {
            ...member,
            goals: member.goals.map((m) => applyMember(m, progress)),
        };
    }
...
function applyProgress<
    T extends { id: string; target: number; progress: number },
>(goal: T, progress: Record<string, number>): T {
